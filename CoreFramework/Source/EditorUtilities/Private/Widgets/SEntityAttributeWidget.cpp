...
void SAttributeListWidget::Construct(const FArguments& InArgs)
{
	struct Local
	{
		static void AttributeToStringArray(const FProperty& Property, OUT TArray< FString >& StringArray)
		{
			...
			if ((Class->IsChildOf(UEntityAttributeSet::StaticClass()) && !Class->ClassGeneratedBy) ||
				(Class->IsChildOf(UAttributeSystemComponent::StaticClass()) && !Class->ClassGeneratedBy))
			{
...
void SEntityAttributeWidget::Construct(const FArguments& InArgs)
{
	FilterMetaData = InArgs._FilterMetaData;
	...
	SAssignNew(ComboButton, SComboButton)
		.OnGetMenuContent(this, &SEntityAttributeWidget::GenerateAttributePicker)
		.ContentPadding(FMargin(2.0f, 2.0f))
		.ToolTipText(this, &SEntityAttributeWidget::GetSelectedValueAsString)
		.ButtonContent()
		[
			SNew(STextBlock)
			.Text(this, &SEntityAttributeWidget::GetSelectedValueAsString)
		];
...
void SEntityAttributeWidget::OnAttributePicked(FProperty* InProperty)
{
...
TSharedRef<SWidget> SEntityAttributeWidget::GenerateAttributePicker()
{
	FOnAttributePicked OnPicked(FOnAttributePicked::CreateRaw(this, &SEntityAttributeWidget::OnAttributePicked));
...
FText SEntityAttributeWidget::GetSelectedValueAsString() const
{
	if (SelectedProperty)
	{
