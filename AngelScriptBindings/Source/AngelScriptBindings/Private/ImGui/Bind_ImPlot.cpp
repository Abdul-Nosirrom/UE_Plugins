...
FAngelscriptBinds::FBind Bind_ImGui_PlotItems(FAngelscriptBinds::EOrder::Late, []
{
	...
	[](const FString& Label, const TArray<FVector2f>& Values, const double BarHeight)
	{
		if (!Values.IsEmpty())
		{
			ImPlot::PlotBars(ToImGui(Label), &Values[0].X, &Values[0].Y, Values.Num(), BarHeight, ImPlotBarsFlags_Horizontal, sizeof(FVector2f));
		}
	});
	...
	[](const FString& Label, const TArray<FVector2D>& Values, const double BarHeight)
	{
		if (!Values.IsEmpty())
		{
			ImPlot::PlotBars(ToImGui(Label), &Values[0].X, &Values[0].Y, Values.Num(), BarHeight, ImPlotBarsFlags_Horizontal, sizeof(FVector2D));
		}
	});
	...
	[](const FString& Label, const TArray<float>& Values)
	{
		if (!Values.IsEmpty())
		{
			ImPlot::PlotInfLines(ToImGui(Label), Values.GetData(), Values.Num(), 0, sizeof(FVector2D));
		}
	});
	...
	[](const FString& Label, const TArray<double>& Values)
	{
		if (!Values.IsEmpty())
		{
			ImPlot::PlotInfLines(ToImGui(Label), Values.GetData(), Values.Num(), 0, sizeof(double));
		}
	});
	...
	[](const FString& Label, const TArray<float>& Values)
	{
		if (!Values.IsEmpty())
		{
			ImPlot::PlotInfLines(ToImGui(Label), Values.GetData(), Values.Num(), ImPlotInfLinesFlags_Horizontal, sizeof(FVector2D));
		}
	});
	...
	[](const FString& Label, const TArray<double>& Values)
	{
		if (!Values.IsEmpty())
		{
			ImPlot::PlotInfLines(ToImGui(Label), Values.GetData(), Values.Num(), ImPlotInfLinesFlags_Horizontal, sizeof(double));
		}
	});
	...
	{
		ImPlot::PlotText(ToImGui(Text), X, Y, ToImGui(Offset), Vertical ? ImPlotTextFlags_Vertical : 0);
	});
